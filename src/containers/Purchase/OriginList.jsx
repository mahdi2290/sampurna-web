...
import * as BaseService from '../../service/Origin/OriginService';
...
const OriginList = () => {
    ...
    const title = "Origin";
    const columnsTable = [
        { field: 'code', header: 'Kode', sortable: false, style: "percent", width: 0.6 },
        { field: 'name', header: 'Nama', sortable: false, style: "percent", width: 1.7 },
        ...
        { field: 'status', header: 'Status', sortable: false, style: "percent", width: 0.3, body: "boolean" },
        { field: 'action', header: '' },
    ];
    ...
    const QueryData = async(page, pageSize, keyword) => {
        ...
        const res = await BaseService.GetAll(params);
    ...
    const actionNew = () => {
        return history.push({
            pathname: "/purchase/master/origin/create",
            state: {}
        });
    }
...
export default React.memo(OriginList, comparisonFn);
