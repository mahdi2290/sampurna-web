...
import * as CrudService from '../../service/Fleet/FleetTypeService';
import { APIBisnisUnit } from "../../components/api/APIMaster";
...
const FleetTypeList = () => {
    ...
    const [spkFlag, setSpkFlag] = useState(false);
    ...
    const onSetModel = (data) => {
        setSpkFlag(data.is_spk === 1 ? true : false);
    ...
    if (localModeStorage === "static" ) {
        columnsTable = [
            { field: 'name', header: 'Nama', sortable: false, style: "percent", width: 6, },
            { field: 'is_spk', header: 'SPK', sortable: false, style: "percent", width: 0.3, body: "boolean" },
            { field: 'action', header: '' },
        ];
    } else {
        columnsTable = [
    ...
    const dynamicForm = (
        <Fragment>
            <form onSubmit={handleSubmit} autoComplete="off">
                <AutoCompleteComp className="field col-12 md:col-12" api={true} validate={true} field="sales_type_id" title="Bisnis Unit" showField="name" errors={errors} touched={touched} models={salesTypeData}
                    queryData={(e) => getSalesType(e)} value={salesTypeSelect} setSelectValue={(e) => setSalesTypeSelect(e)} onChange={(e) => { setSalesTypeSelect(e.value); values.sales_type_id = e.value; }} />
                <InputTextComp className="field col-12 md:col-12" validate={true} field="name" title="Nama" value={values.name} onChange={handleChange} onBlur={handleBlur} errors={errors} touched={touched} />
                <InputSwitchComp className="field col-12 md:col-12" validate={true} field="is_spk" title="SPK" checked={spkFlag} onChange={(e) => setSpkFlag(e.value)} onBlur={handleBlur} errors={errors} touched={touched} />
            </form>
        </Fragment>
    )
...
export default React.memo(FleetTypeList, comparisonFn);
