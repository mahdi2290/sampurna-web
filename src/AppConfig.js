...
export const AppConfig = (props) => {
    ...
    useEffect(()=> {
        ...
        if (scaleStorage && Auth.isAuthenticated()) {
            setScale(scaleStorage);
        } else {
            setScale(12);
        }
        ...
        let schemeStorage = localStorage.getItem("scheme");
        
        if (schemeStorage) {
            props.onColorModeChange(schemeStorage);
        } else {
    ...
    const decrementScale = () => {
        ...
        let scaleStorage = parseInt(scale) - 1;
        
        localStorage.setItem("scale", scaleStorage);
    }
    ...
    return (
        <div ref={config} className={configClassName} id={"layout-config"}>
            ...
            <div className="layout-config-content">
                <h5 className="mt-0">Font Size : <b>{scale}</b></h5>
